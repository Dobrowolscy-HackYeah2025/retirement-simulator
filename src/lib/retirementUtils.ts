...
const getContributionRateFromData = (year: number): number => {
  ...
  if (yearIndex >= 0 && yearIndex < parametryData['parametry roczne'].rows.length) {
    const parametry = parametryData['parametry roczne'].rows[yearIndex];
    const employeeRate = parametry?.['stopa składki na ubezpieczenie emerytalne finansowanej przez pracownika'] || 0;
    const employerRate = parametry?.['stopa składki na ubezpieczenie emerytalne finansowanej przez pracodawcę'] || 0;
